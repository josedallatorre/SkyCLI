from utils.conversion import toDigits, fromDigits
import pytest

class TestConversion:
    def test_bideractional_conversion(self):
    ...
    def test_zero_toDigits(self):
        assert toDigits(0, 2) == [0]
    
    def test_negative_number_toDigits(self):
    ...
    def test_zero_base_fromDigits(self):
        with pytest.raises(ValueError):
            fromDigits([1,0,1], 0)
    
    def test_negative_base_fromDigits(self):
        with pytest.raises(ValueError):
            fromDigits([1,0,1], -2)
